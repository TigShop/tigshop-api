...
class UserBalanceLogService extends BaseService
{
    ...
    public function accountChange(int $user_id, array $data = [])
    {
        ...
        $user_info->balance = $data["change_type"] == 1 ? $user_info->balance + $user_balance_log["balance"] : $user_info->balance - $user_balance_log["balance"];
        $user_info->frozen_balance = $data["change_type"] == 1 ? $user_info->frozen_balance + $user_balance_log["frozen_balance"] : $user_info->frozen_balance - $user_balance_log["frozen_balance"];
        $user_info->save();
    }
}
