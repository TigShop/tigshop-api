...
class SignInSettingService extends BaseService
{
    ...
    public function getDetail(int $id): SignInSetting
    {
        ...
        if (!$result) {
            throw new ApiException(/** LANG */'积分签到不存在');
        }
    ...
    public function updateSignInSetting(int $id, array $data): bool
    {
        ...
        AdminLog::add('更新积分签到:' . $this->getName($id));
        return $result !== false;
    }
     ...
     * @throws ApiException
     */
    public function updateSignInSettingField(int $id, array $data)
    {
        if (!$id) {
            throw new ApiException(/** LANG */'#id错误');
        }
